...
$tblname = 'mails';
$sql_query = "SELECT * FROM $tblname WHERE mailtype='intro'";
		...
		if($row2['status'] == 'introduced') {
			...
			if($row['remaining'] == 0) {
				...
				*/
				...
				$sql_query = "UPDATE $tblname SET remaining=$remaining WHERE id=$clientid AND mailtype='intro'";
				$conn->query( $sql_query );
